...
router.post('/criterion', async (req, res) => {
  try {
    ...
    console.log(
      `[Parser] Parsing single criterion ${criterion.id} with cluster ${cluster} [model: ${modelUsed}]`
    );
    const parseResult = await parser.parseCriterion(criterion, cluster, modelUsed);
...
router.post('/batch', async (req, res) => {
  try {
    ...
    const modelUsed = 'claude-sonnet-4-5-20250929';
    console.log(
      `[Parser] Parsing batch of ${criteria.length} criteria with cluster ${cluster} [model: ${modelUsed}]`
    );
    const results = await parser.parseBatch(criteria, cluster, { ...options, model: modelUsed });
...
function detectClusterFromIds(criteria) {
  ...
  for (const criterion of criteria) {
    const id = String(criterion.id || criterion.criterion_id || '');
...
router.post('/upload', async (req, res) => {
  try {
    ...
    // Method 0: Try extracting from nested cluster object in members (e.g., cluster.name)
    if (!clusterType && criteria.length > 0 && criteria[0].cluster) {
      const firstClusterName = criteria[0].cluster.name;
      if (firstClusterName) {
        clusterType = detectClusterFromClusterName(firstClusterName);
        if (clusterType) {
          detectionMethod = 'nested_cluster_name';
          console.log(
            `[Parser] Auto-detected cluster type: ${clusterType} from nested cluster.name: "${firstClusterName}"`
          );
        }
      }
    }
    ...
    if (!clusterType && data.cluster_name) {
      ...
      if (clusterType) {
        ...
        console.log(
          `[Parser] Auto-detected cluster type: ${clusterType} from cluster_name: "${data.cluster_name}"`
        );
      }
    }
    ...
    if (!clusterType && filename) {
      ...
      if (clusterType) {
        ...
        console.log(
          `[Parser] Auto-detected cluster type: ${clusterType} from filename: "${filename}"`
        );
      }
    }
    ...
    if (!clusterType) {
      ...
      if (clusterType) {
        ...
        console.log(
          `[Parser] Auto-detected cluster type: ${clusterType} from ${criteria.length} criteria IDs`
        );
      }
    }
    ...
    if (!clusterType) {
      const availableClusters = UniversalParserV2.getClusterCodes();
      const foundClusterName =
        criteria.length > 0 && criteria[0].cluster ? criteria[0].cluster.name : null;
      return res.status(400).json({
        error: 'Cannot determine cluster type. Please manually select from available clusters.',
        availableClusters: availableClusters,
        foundClusterName: foundClusterName,
        hint: `Your file has cluster name: "${foundClusterName}". Please select one of the available clusters manually.`,
      });
    }
...
router.post('/job/start', async (req, res) => {
  try {
    ...
    const db = getDatabase();
    if (db) {
      const budgetConfig = await db.getAsync('SELECT value FROM config WHERE key = ?', [
        'parser_budget_limit',
      ]);
    ...
    // Update database
    if (db) {
      await db.runAsync('UPDATE parser_jobs SET status = ?, modelId = ? WHERE id = ?', [
        'running',
        job.model,
        jobId,
      ]);
...
router.post('/job/pause', async (req, res) => {
  try {
    ...
    const db = getDatabase();
    if (db) {
      await db.runAsync('UPDATE parser_jobs SET status = ? WHERE id = ?', ['paused', jobId]);
    }
...
router.post('/job/resume', async (req, res) => {
  try {
    ...
    const db = getDatabase();
    if (db) {
      await db.runAsync('UPDATE parser_jobs SET status = ? WHERE id = ?', ['running', jobId]);
    }
...
router.get('/job/:jobId/results', async (req, res) => {
  try {
    ...
    const results = await db.allAsync('SELECT * FROM parsed_criteria WHERE jobId = ?', [jobId]);
...
router.get('/balance', async (req, res) => {
  try {
    ...
    let usedAmount = 0;
    let budgetLimit = 100.0; // Default $100
...
router.delete('/cache/:criterionId', async (req, res) => {
  try {
    ...
    await db.runAsync('DELETE FROM parsed_criteria WHERE criterionId = ?', [criterionId]);
...
async function parseJobInBackground(jobId) {
  ...
  console.log(
    `[Parser] Starting job ${jobId}, parseLimit=${job.maxCount || 'ALL'}, criteria=${job.criteria.length}`
  );
  ...
  console.log(`[Parser] Using model: ${job.model}`);
  console.log(
    `[Parser] Will parse up to ${parseLimit} NEW criteria (total in file: ${job.criteria.length})`
  );
  ...
  for (let i = 0; i < job.criteria.length; i++) {
    ...
    try {
      console.log(
        `[Parser] Parsing criterion ${i + 1}/${job.criteria.length}: ${criterion.id} (new: ${parsedInThisSession + 1}/${parseLimit}) [model: ${job.model}]`
      );
      // Parse the criterion - returns { criterion, usage }
      const parseResult = await parser.parseCriterion(criterion, job.clusterType, job.model);
      ...
      console.log(
        `[Parser] Cost: $${costUsd.toFixed(4)} [model: ${job.model}] (in:${inputTokens}, out:${outputTokens}, cache_r:${cacheReadTokens}, cache_w:${cacheWriteTokens})`
      );
      ...
      if (db) {
        try {
          ...
          const clusterType =
            job.clusterType && job.clusterType.trim() !== '' ? job.clusterType : 'UNKNOWN';
          const rawInput =
            criterion.raw_text && criterion.raw_text.trim() !== '' ? criterion.raw_text : 'N/A';
        ...
        try {
          await db.runAsync(
            ...
            VALUES (?, ?, ?, ?, ?, ?)
          `,
            [new Date().toISOString(), job.model, inputTokens, outputTokens, costUsd, jobId]
          );
        } catch (dbError) {
