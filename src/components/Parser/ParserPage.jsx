...
export default function ParserPage() {
  ...
  const fetchJobStatus = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok && !data.error) {
        ...
        const cost =
          typeof data.actualCost === 'string' ? parseFloat(data.actualCost) : data.actualCost || 0;
  ...
  const handleStartParsing = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const effectiveTotal = parseLimit
          ? Math.min(parseInt(parseLimit, 10), uploadData.needsParsing)
          : uploadData.needsParsing;
  ...
  const handleDownloadResults = () => {
    ...
    // Get unique cluster types to include in export
    const clusterTypes = [...new Set(results.map((r) => r.clusterType))];
    const clusterInfo = {};
    clusterTypes.forEach((code) => {
      clusterInfo[code] = CLUSTER_NAMES[code] || code;
    });
    ...
    const exportData = {
      ...
      results: results.map((r) => ({
        ...
        clusterName: CLUSTER_NAMES[r.clusterType] || r.clusterType,
    ...
    const clusterNames = clusterTypes.map((code) => code).join('-');
    a.download = `clinical-trial-criteria-parsed_${clusterNames}_${date}T${time}.json`;
    ...
    <div className="parser-page">
      <header className="parser-header">
        ...
        <div className="header-status">
          {parserVersion && <span className="parser-version">Parser v{parserVersion}</span>}
      ...
      <section className="parser-section">
        ...
        {uploadData && (
          <div className="upload-summary">
            <p>
              <strong>{uploadData.criteriaCount}</strong> criteria total
            </p>
            <p>
              <strong>{uploadData.alreadyParsed}</strong> already parsed
            </p>
            <p>
              <strong>{uploadData.needsParsing}</strong> need parsing
            </p>
          </div>
        )}
      </section>
      ...
      {estimate && (
        <section className="parser-section estimate-section">
          ...
          <p>
            <strong>Cost:</strong> $
            {typeof estimate.estimatedCost === 'number'
              ? estimate.estimatedCost.toFixed(3)
              : `${estimate.estimatedCost.min.toFixed(2)} - ${estimate.estimatedCost.max.toFixed(2)}`}
          </p>
          <p>
            <strong>Time:</strong>{' '}
            {typeof estimate.estimatedTime === 'number'
              ? `~${estimate.estimatedTime}`
              : `${estimate.estimatedTime.min} - ${estimate.estimatedTime.max}`}{' '}
            seconds
          </p>
      ...
      <section className="parser-section controls-section">
        ...
        {jobStatus === 'running' && (
          <button className="btn btn-warning" onClick={handlePauseParsing}>
            Pause
          </button>
        )}

        {jobStatus === 'paused' && (
          <button className="btn btn-success" onClick={handleResumeParsing}>
            Resume
          </button>
        )}
        ...
        {jobStatus && (
          <div className="progress-section" data-testid="progress-section">
            ...
            <div className="progress-stats">
              <span>
                {progress.parsed} / {progress.total} parsed
              </span>
                ...
                <ul style={{ margin: '0.5rem 0 0 1.5rem', padding: 0 }}>
                  {parseErrors.slice(0, 3).map((err, idx) => (
                    <li key={idx} style={{ fontSize: '0.9rem' }}>
                      <code>{err.criterionId}</code>:{' '}
                      {err.error.includes('credit balance')
      ...
      {jobStatus === 'completed' && results.length === 0 && (
        <section
          ...
          style={{
            backgroundColor:
              progress.skipped > 0 && parseErrors.length === 0 ? '#fff3cd' : '#f8d7da',
      ...
      <section className="parser-section balance-section">
        <h2>API Usage</h2>
        <p>
          <strong>Total Spent:</strong> ${balance.totalSpent.toFixed(2)}
        </p>
      </section>
          ...
          <ul className="history-list">
            {history.map((job) => (
              <li key={job.id}>
                ...
                <span>
                  {job.parsedCount}/{job.totalCriteria} criteria
                </span>
